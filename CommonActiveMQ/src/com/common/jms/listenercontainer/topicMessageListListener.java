...
@Slf4j
public class topicMessageListListener {
	@Resource 
			...
			if(null!=list && 0!=list.size())
			{
				for(int i=0;i<list.size();i++)
				{
					...
					if(null != session)
					{
						...
						strSql+="WHERE interface.al_servertype=? and computer.ac_name=? and computer.id=? and interface.al_destinationtype='topic'";
						try {
						...
						if(null!=listInfo)
						{
							for(int x=0;x<listInfo.size();x++)
							{
								...
								{
									computerinfo=mapLink.get("computerinfo").toString()+"_topic";
								}
								...
								{
									log.info("=============TopicMSGListener start====================");
									log.info("TopicMSGDestinationName:"+computerinfo+":"+listname);
									log.info("TopicMSGBeanName:"+classinfo);
									topic=session.createTopic(computerinfo+":"+listname);
