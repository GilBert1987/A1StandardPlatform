...
function JavaLargeFileUploader() {
	var globalServletMapping = "../../platform/upload/javaLargeFile";
	var uploadServletMapping = "../../platform/upload/javaLargeFileAsync";
	...
	errorMessages[0] = "因不明原因请求失败，如果问题持续存在，请与管理员联系。";
	errorMessages[1] = "请求不是multipart类型。";
	errorMessages[2] = "在请求中没有找到文件。";
	errorMessages[3] = "CRC32部分验证失败。";
	errorMessages[4] = "由于缺少参数，该请求不能处理。";
	errorMessages[5] = "无法检索配置。";
	errorMessages[6] = "未选定文件，请选择至少一个文件！";
	errorMessages[7] = "恢复文件上传与前一部分的最后一部分是无效的。";
	errorMessages[8] = "上传一个文件时出错";
	errorMessages[9] = "达到最大数量的并发上传，上传排队等待一个完成。";
	errorMessages[10] = "发生意外。重试…";
	errorMessages[11] = "连接丢失。自动重试。";
	errorMessages[12] = "您没有权限来执行此操作。";
	errorMessages[13] = "Firebug是启用的，你如果不禁用它会有同时上传的问题。";
	errorMessages[14] = "文件损坏。出现未知错误，该文件已损坏。通常的原因是，该文件已被修改，在上传过程中。请清除它并重新上传。";
	errorMessages[15] = "文件当前被锁定，重试一下…";
	errorMessages[16] = "上传暂时禁用，重试一下…";
	...
	this.setGlobalServletMapping = function(value){
		globalServletMapping=value;
	};

	this.setUploadServletMapping = function(value){
		uploadServletMapping=value;
	};
	...
	this.initialize = function (initializationCallback, exceptionCallback, optionalClientOrJobIdentifier) {
		...
		$.ajax({
			url: javaLargeFileUploaderHost + globalServletMapping + "?action=getConfig" + appended,
			type:"GET",
			cache:false,
				...
				if (data) {
					...
					if (!jQuery.isEmptyObject(data.pendingFiles)) {
						...
						$.each(data.pendingFiles, function(key, pendingFile) {
							...
							pendingFile.originalFileName=decodeURIComponent(pendingFile.originalFileName);
	...
	function processNewFiles(newFiles) {
		...
		for (pendingFileId in newFiles) {
			...
			fileForPost.fileName=encodeURI(pendingFile.originalFileName);
			...
			extractCrcFirstSlice(pendingFile.blob, function(crc, blob) {
				...
				if (crcsCalculated == jsonVersionOfNewFiles.length) {
					$.ajax({
						url: javaLargeFileUploaderHost + globalServletMapping + "?action=prepareUpload",
						data:{newFiles: JSON.stringify(jsonVersionOfNewFiles)},
						type: "GET",
						dataType:'json',
						cache:false,
						contentType:"application/x-www-form-urlencoded;charset=UTF-8",
	...
	function go(pendingFile) {
		...
		reader.onloadend = function(e) {
		    if (e.target.readyState == FileReader.DONE) { // DONE == 2
				...
				//then open
				xhr.open('POST', javaLargeFileUploaderHost + uploadServletMapping + '?action=upload&fileId=' + pendingFile.id + '&crc=' + decimalToHexString(digest)+'&time='+nowTime, true);
