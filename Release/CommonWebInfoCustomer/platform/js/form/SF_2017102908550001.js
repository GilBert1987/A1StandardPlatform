				...
				if("SequenceFlow"==eleObj.stencil.id)
				{
					$(paths).each(function(index, element) {
                        if(eleObj.resourceId==element.from || eleObj.resourceId==element.to)
						{
							...
							element.textX=(null!=eleObj.properties.textX?eleObj.properties.textX:0);
							element.textY=(null!=eleObj.properties.textY?eleObj.properties.textY:0);
...
function fn_saveFormData(jsonObj)
{
	...
	{
		$("#hjsinfo").val(encodeURIComponent(decodeURIComponent(jsonObj.jsinfo)).replace(/"/g, "&quot;").replace(/'/g, "&apos;"));
	}
	...
	{
		$("#hdynamicform").val(encodeURIComponent(decodeURIComponent(jsonObj.dynamicform)).replace(/"/g, "&quot;").replace(/'/g, "&apos;"));
	}
}
...
function selPath(pathObj){
	...
	tabInfo+='<option value="1" '+("1"==pathInfo.properties.linetype?"selected='selected'":"")+'>[直线]可调纵向折线</option>';
	tabInfo+='<option value="2" '+("2"==pathInfo.properties.linetype?"selected='selected'":"")+'>[直线]可调横向折线</option>';
	tabInfo+='<option value="3" '+("3"==pathInfo.properties.linetype?"selected='selected'":"")+'>虚线</option>';
	tabInfo+='<option value="4" '+("4"==pathInfo.properties.linetype?"selected='selected'":"")+'>[虚线]可调纵向折线</option>';
	tabInfo+='<option value="5" '+("5"==pathInfo.properties.linetype?"selected='selected'":"")+'>[虚线]可调横向折线</option>';
	tabInfo+='</select>';
	tabInfo+='</td>';
	tabInfo+='</tr>';
	tabInfo+='<tr>';
...
function retPostData(callFun){
	...
	eleList=dataList.rectList;
	phList=dataList.pathList;
...
function setPostData(initData,eleList,pathList){
	...
	$(eleList).each(function(i,ele) {
		...
		shareObj={"x":$(eleRect).attr("x"),"y":$(eleRect).attr("y"),"width":$(eleRect).attr("width"),"height":$(eleRect).attr("height")};
		childData.bounds={"lowerRight":{"x":Math.round(shareObj.x+shareObj.width),"y":Math.round(shareObj.y+shareObj.height)},"upperLeft": {"x":Math.round(shareObj.x),"y":Math.round(shareObj.y)}};
		$(pathList).each(function(ipath,elepath) {
            if("path" ==elepath.path.nodeName && elepath.data.from==eleData.id)
			{
	...
	$(pathList).each(function(i,ele) {
		...
		if("path"==elePath.nodeName)
		{
			...
			childData.properties=(eleData.properties==null?{}:eleData.properties);
			childData.properties.conditionsequenceflow=encodeURIComponent(decodeURIComponent(eleData.properties.conditionsequenceflow)).replace(/"/g, "&quot;").replace(/'/g, "&apos;");
			childData.stencil={"id":"SequenceFlow"};
			childData.childShapes=[];
			childData.outgoing=outgoing;
			childData.bounds={"lowerRight":{"x":Math.round(shareObj.x),"y":Math.round(shareObj.y)},"upperLeft": {"x":Math.round(shareObj.x-shareObj.width),"y":Math.round(shareObj.y-shareObj.height)}};
...
function getInitDataById(id){
	var jsonObj;
	var jsonValue;
	jsonValue=$("#hInitWFData").val();
	jsonObj=JSON.parse(jsonValue);
	
}
function modifyTableByData(rectObj){
	var properties;
	var baseData;
	var roleData;
	var userData;
...
function fn_saveStart(){
	...
	if(""!=$("#startFormkey").val() && ""!=$("#endFormkey").val() && ""!=roleInfo && "[]"!=roleInfo)
	{
		...
		properties.formkeydefinition=encodeURIComponent("{\"startcode\":\""+$("#startFormkey").val()+"\",\"endcode\":\""+$("#endFormkey").val()+"\"}").replace(/"/g, "&quot;").replace(/'/g, "&apos;");
		properties.formreference="",
		properties.formproperties="";
		jsonObj.properties=properties;
...
function fn_saveUserTask(){
	...
	if(""!=baseInfo && ""!=userInfo && ""!=roleInfo)
	{
		...
		if(0!=$(userList).size())
		{
			...
			properties.usertaskassignment={"totalCount":$(userList).size(),"items":userList,"assignment":userAssignmentList};
