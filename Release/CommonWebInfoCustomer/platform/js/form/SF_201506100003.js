$(document).ready(function(e) {
	...
	if(""!=initValue && null!=initValue)
	{
		...
		if(""!=initData)
		{
			...
			if(null!=initData)
			{
				$("#controls").val(JSON.stringify(initData));
			}
		}
		...
		else
		{
			...
			$(conlist).each(function(index, conele) {
				...
				if("btnsubInitWF"!=conele.control[0].id)
				{
        			list.push({"id":conele.control[0].id,"show":[{"authinfo":authinfo,"showtype":"2"}]});
				}
				else
				{
					list.push({"id":conele.control[0].id,"show":[{"authinfo":authinfo,"showtype":"0"}]});
				}
        		list.push({"id":conele.control[0].id,"show":[{"authinfo":authinfo,"showtype":"2"}]})
    		});
			$(conlist).each(function(index, conele) {
...
function initAuthoritys(){
	...
	$(list).each(function(i,item){
		if(item.authority[0]!=null)
		{
			strTemp="<tr>";
			strTemp+="<td width='10%'>";
			strTemp+="<input name='selauthority' type='radio' value='"+item.authority[0].code+"' onclick='clickTable(this);'>";
			strTemp+="</td>";
			strTemp+="<td  width='90%'>";
...
function fn_delAuthorityItem(){
	...
	if($('input[name="selobj"]:checked').size()!=0)
	{
		...
		{
			tabobj=$($($(jsonobj[i]).parent()).parent()).parent()[0];
			if(tabobj.rows.length > 1)
			{
				item=JSON.parse($(jsonobj[i]).val());
				authitemid=$($(jsonobj[i]).parent().parent().parent().parent().parent().parent().parent().parent().parent().parent().children().get(1)).text();
...
function addAuthorityItem(item){
	...
	var authitemid;
	var bladd;
	$('input[name="selauthobj"]:checked').each(function(i, ele) {
        bladd=false;
		authitemid=$(ele).val();
		authority=JSON.parse($('#authority').val());
		$(authority).each(function(ci,citem){
...
function fn_save(){
	...
	list=JSON.parse($('#authoritys').val());
	parent.window.fn_saveRole(list);
	closeWin();
}