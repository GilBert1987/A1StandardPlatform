$(document).ready(function(e) {
	...
	$(consList).each(function(index, ele) {
        if(btnId==ele.control[0].id)
		{
			...
			{
				btnInfo.control[0].icon=btnInfo.control[0].icon.replace(/&nbsp;/g, " ");
			}
			if(null!=btnInfo.control[0].strstyle)
			{
				btnInfo.control[0].strstyle=btnInfo.control[0].strstyle.replace(/&nbsp;/g, " ");
			}
			if(null!=btnInfo.control[0].strclass)
			{
				btnInfo.control[0].strclass=btnInfo.control[0].strclass.replace(/&nbsp;/g, " ");
			}
		...
		$(btnObj.firefunction).each(function(index, ele) {
			...
			if(null!=ele.parm)
			{
				for(var i=0;i<ele.parm.length;i++)
				{
					...
					tabText+='</td>';
					tabText+='<td style="width:75%;">';
					tabText+='<input type="text" style="width:98%;" value="'+decodeURIComponent(ele.parm[i].value).replace(/"/g, "&quot;").replace(/'/g, "&apos;")+'"/>';
					tabText+='</td>';
					tabText+='</tr>';
				}
			}
...
function initClientTable(btnObj,btnInfo){
	...
	if(null!=btnObj.clientfunction){
		$(btnObj.clientfunction).each(function(index, ele) {
			tabText='<tr>';
			tabText+='<td style="width:70%;">';
			tabText+='<input type="text" style="width:98%;" value="'+decodeURIComponent(ele.function).replace(/"/g, "&quot;").replace(/'/g, "&apos;")+'"/>';
			tabText+='</td>';
			tabText+='<td style="width:10%;">';
			tabText+='<a href="#" onclick="fn_ClientTrUp(this);">上移</a>';
			tabText+='</td>';
...
function addFireFun(ele){
	...
	tabText+='</td>';
	tabText+='<td style="width:75%;">';
	tabText+='<input type="text" style="width:90%;" value="'+ele.fireclass+'"/>';
	tabText+='</td>';
	tabText+='</tr>';
	tabText+='<tr>';
	tabText+='<td style="width:25%;">';
	...
	tabText+='</td>';
	tabText+='<td style="width:75%;">';
	tabText+='<input type="text" style="width:90%;" value="'+ele.method+'"/>';
	tabText+='</td>';
	tabText+='</tr>';
	tabText+='<tr>';
	tabText+='<td style="width:100%;" colspan="2">';
	...
	if(null!=ele.parm)
	{
		for(var i=0;i<ele.parm.length;i++)
		{
			...
			tabText+='</td>';
			tabText+='<td style="width:75%;">';
			tabText+='<input type="text" value="'+ele.parm[i].key+'" readonly="readonly" style="width:90%;"/>';
			tabText+='</td>';
			tabText+='</tr>';
			tabText+='<tr>';
			tabText+='<td style="width:25%;">';
			...
			tabText+='</td>';
			tabText+='<td style="width:75%;">';
			tabText+='<input type="text" value="'+decodeURIComponent(ele.parm[i].value).replace(/"/g, "&quot;").replace(/'/g, "&apos;")+'"  style="width:90%;" />';
			tabText+='</td>';
			tabText+='</tr>';
		}
	}
		...
		if(true==fn_checkControl(btnid))
		{
			...
			if(null!=btnList){
				...
				for(var i=0;i<$('#clientFun')[0].rows.length;i++)
				{
					...
					if(""!=clientText){
						clientText=encodeURIComponent(decodeURIComponent(clientText));
				...
				for(var j=0;j<$('#serverFun')[0].rows.length;j++)
				{
					...
					if(null!=fireTab)
					{
						...
						for(var i=0;i<paramTab.rows.length;i++)
						{
							...
							{
								paramText+='"value":"'+encodeURIComponent(decodeURIComponent($($(paramTab.rows[i].cells[1]).children()[0]).val()))+'"},';
							}
						}
						if(fireTab.rows.length>0)
						{
			...
			if(null!=consList && null!=btnList){
				parent.$("#hid").val(btnObj.id);
				parent.$("#htitle").val(btnObj.tag);
