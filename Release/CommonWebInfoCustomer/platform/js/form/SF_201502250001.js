...
function fn_deptuser(deptid){
	var postData;
	var sqlInfo;
	deptid=(deptid==null?"":deptid);
	sqlInfo="SELECT DISTINCT sc_user.id,sc_user.realname FROM sc_post_org JOIN sc_user_post ON sc_post_org.id=sc_user_post.por_id JOIN sc_user ON sc_user_post.user_id=sc_user.id WHERE sc_post_org.org_id='"+deptid+"'";
...
function fn_roleuser(roleid){
	var postData;
	var sqlInfo;
	roleid=(roleid==null?"":roleid);
	sqlInfo="SELECT DISTINCT sc_user.id,sc_user.realname FROM sc_user_role JOIN sc_user ON sc_user_role.user_id=sc_user.id WHERE sc_user_role.role_id='"+roleid+"'";
	postData={"type":"sql","dataname":"shiro","info":sqlInfo};
	$.ajax({
		url: "../../ajax/query/search",
		type: "POST",
...
function fn_selList(list){
	...
	$(list).each(function(index, ele) {
         option='<option value="'+ele.id+'">'+ele.realname+'</option>';
