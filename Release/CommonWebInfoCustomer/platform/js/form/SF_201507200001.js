...
function initControls(){
	...
	$(list).each(function(i,item){
		if(null!=item.control[0] && null != item.control[0].id && authobj != null)
		{
			...
			strTemp="<tr>";
			strTemp+="<td width='20%'>";
			strTemp+="<input name='selauthobj' type='radio' value='"+item.control[0].id+"' onclick='clickItem(this);' />";
			strTemp+="</td>";
			strTemp+="<td width='30%'>";
			...
			if(null != controlauthObj && null!= controlauthObj.show)
			{
				$(controlauthObj.show).each(function(ci,citem){
					...
					strTable+="<input name='selobj' type='radio' value='{\"showtype\":\""+citem.showtype+"\",\"authinfo\":\""+encodeURIComponent(citem.authinfo)+"\"}' />";
					strTable+="</td>";
