...
import com.common.jasper.xmlparser.XMLEncodingDetector;
...
class ParserController  implements TagConstants {
    ...
    throws FileNotFoundException, JasperException, IOException {
        ...
        if (isXml) {
            ...
            // Standard syntax
            try {
            	if(this.getCompiler().getJspContent()!=null)
            	{
            		in_nocode=new ByteArrayInputStream(this.getCompiler().getJspContent().toString().getBytes(sourceEnc));
            		inStreamReader=new InputStreamReader(in_nocode,sourceEnc);
            	}
            	else
            	{
    ...
    throws JasperException, IOException {
        ...
        if (isExternal && !isXml) {
        	...
        	if(compiler!=null && "".equals(compiler.getSourceEnc())==false)
        	{
        		ret = new Object[]{(compiler.getSourceEnc()==null?"UTF-8":compiler.getSourceEnc()),false,false,0};
        	}
        	else
        	{
        		ret = XMLEncodingDetector.getEncoding(absFileName,
                        jarFile, ctxt, err);
        	}
        ...
        if(this.getCompiler().getJspContent()!=null)
        {
        	...
        	{
	        	in_nocode=new ByteArrayInputStream(this.getCompiler().getJspContent().toString().getBytes(sourceEnc));
	        	in_reader=new InputStreamReader(in_nocode,sourceEnc);
	        	jspReader=new JspReader(ctxt, absFileName, sourceEnc, in_reader,err);
        	}
        	finally{
        		if(in_reader!=null)
        		{
