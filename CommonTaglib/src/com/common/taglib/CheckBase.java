...
public class CheckBase extends TagSupport implements TagShow{

	/**
	 * 
	 */
	private static final long serialVersionUID = -5801651341377502021L;
	private static Logger log = Logger.getLogger(CheckBase.class);
	...
	public int doEndTag() throws JspException {
		...
		try {
			input+=createCheckbox((HttpServletRequest)pageContext.getRequest());
			out.println(input);
		} catch (IOException e) {
			log.error(e);
			e.printStackTrace();
	...
	private String createCheckbox(HttpServletRequest request){
		StringBuilder value;
		List<Map> list;
		Map map;
		Iterator iterObj;
		...
		if(null!=showtype && 0!=showtype)
		{
			...
			if(null!=list)
			{
				if(2==showtype)
				{
					if(null!=this.dataBind){
						value.append("<input type='hidden' name='" + this.dataBind +"' value='"+localComService.dataBindInfo(request, this.dataBind)+"'/> ");
					}
				}
				for(int i=0;i<list.size();i++)
				{
					...
					if(2==showtype)
					{
						...
						{
							value.append("<input type='checkbox' coltype='" + (this.backtype==null?"":this.backtype) + "' ");
							value.append("id='"+id+"_"+i+"' ");
							value.append(" "+("true".equals(required)?"required":"")+" ");
							value.append(" showname=\""+infoname+"\" ");
							value.append(" value='" + strCode+"'");
							value.append(" dataBind='" + (null==this.dataBind?"":this.dataBind)+"'");
							dataBindInfo(value,strCode,1);
							value.append(" onclick='"+(null==clientCheckFun?"fn_clickCheckBox(this);":clientCheckFun)+"' ");
							value.append(" />");
